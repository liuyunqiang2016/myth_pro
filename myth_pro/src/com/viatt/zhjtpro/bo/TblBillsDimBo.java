package com.viatt.zhjtpro.bo;


public class TblBillsDimBo extends BaseBo {

	/**
	 * 序列化
	 */
	private static final long serialVersionUID = -8492093573185668921L;
	private String billCode;
	private String itemCode;
	private String roomCode;
	private String billAmt;
	private String jfDate;
	private String jzDate;
	private String isPay;
	private String commID;
	private String areaID;
	private String unitID;
	private String buldID;
	/**
	 * @return the commID
	 */
	public String getCommID() {
		return commID;
	}
	/**
	 * @param commID the commID to set
	 */
	public void setCommID(String commID) {
		this.commID = commID;
	}
	/**
	 * @return the areaID
	 */
	public String getAreaID() {
		return areaID;
	}
	/**
	 * @param areaID the areaID to set
	 */
	public void setAreaID(String areaID) {
		this.areaID = areaID;
	}
	/**
	 * @return the unitID
	 */
	public String getUnitID() {
		return unitID;
	}
	/**
	 * @param unitID the unitID to set
	 */
	public void setUnitID(String unitID) {
		this.unitID = unitID;
	}
	/**
	 * @return the buldID
	 */
	public String getBuldID() {
		return buldID;
	}
	/**
	 * @param buldID the buldID to set
	 */
	public void setBuldID(String buldID) {
		this.buldID = buldID;
	}
	/**
	 * @return the billCode
	 */
	public String getBillCode() {
		return billCode;
	}
	/**
	 * @param billCode the billCode to set
	 */
	public void setBillCode(String billCode) {
		this.billCode = billCode;
	}
	/**
	 * @return the itemCode
	 */
	public String getItemCode() {
		return itemCode;
	}
	/**
	 * @param itemCode the itemCode to set
	 */
	public void setItemCode(String itemCode) {
		this.itemCode = itemCode;
	}
	/**
	 * @return the roomCode
	 */
	public String getRoomCode() {
		return roomCode;
	}
	/**
	 * @param roomCode the roomCode to set
	 */
	public void setRoomCode(String roomCode) {
		this.roomCode = roomCode;
	}
	/**
	 * @return the billAmt
	 */
	public String getBillAmt() {
		return billAmt;
	}
	/**
	 * @param billAmt the billAmt to set
	 */
	public void setBillAmt(String billAmt) {
		this.billAmt = billAmt;
	}
	/**
	 * @return the jfDate
	 */
	public String getJfDate() {
		return jfDate;
	}
	/**
	 * @param jfDate the jfDate to set
	 */
	public void setJfDate(String jfDate) {
		this.jfDate = jfDate;
	}
	/**
	 * @return the jzDate
	 */
	public String getJzDate() {
		return jzDate;
	}
	/**
	 * @param jzDate the jzDate to set
	 */
	public void setJzDate(String jzDate) {
		this.jzDate = jzDate;
	}
	/**
	 * @return the isPay
	 */
	public String getIsPay() {
		return isPay;
	}
	/**
	 * @param isPay the isPay to set
	 */
	public void setIsPay(String isPay) {
		this.isPay = isPay;
	}
}
